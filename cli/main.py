...
@cli.command()
@click.option(
    "--output", "-o", default=".", help="Output directory for fluxwatch config"
)
...
@cli.command()
@click.option(
    "--redis-url", envvar="FLUXWATCH_REDIS_URL", default="redis://localhost:6379"
)
...
@click.option(
    "--channel",
    type=click.Choice(["slack", "pagerduty", "email", "webhook"]),
    default="webhook",
)
@click.option(
    "--target", default="http://localhost:9095", help="Channel target URL/address"
)
